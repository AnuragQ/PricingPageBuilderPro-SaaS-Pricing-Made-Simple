...
async function update(req, res) {
  ...
  // Find widget by widget_name and update it with the request body
  Widget.findOneAndUpdate({ widget_id: req.params.widgetId
}
    , updateFields
    , { new: true })
    .then((widget) => {
      if (!widget) {
        return res.status(404).send({
          message: "Widget not found with name " + req.body.name,
        });
      }
      res.send(widget);
    })
    .catch((err) => {
      ...
      return res.status(500).send({
        message: "Error updating widget with name " + req.body.name,
      });
    });
}
