...
async function createCheckoutSession(req, res) {
  try {
    ...
    const stripe = require("stripe")(user && user.stripe_key || process.env.STRIPE_PRIVATE_KEY);
    ...
    const sessionItems = await Promise.all(req.body.items.map(async item => {
      const plan = await button_model.findOne({
        button_id: item.id
      });
      return {
        price_data: {
