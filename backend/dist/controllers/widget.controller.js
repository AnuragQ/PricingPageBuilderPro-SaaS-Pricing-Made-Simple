...
async function findOne(req, res) {
  Widget.findById(req.params.widgetId).then(widget => {
    if (!widget) {
...
async function update(req, res) {
  ...
  Widget.findByIdAndUpdate(req.params.widgetId, updateFields, {
    new: true
  });
}
...
async function remove(req, res) {
  Widget.findByIdAndRemove(req.params.widgetId).then(widget => {
    if (!widget) {
      return res.status(404).send({
        message: "Widget not found with id " + req.params.widgetId
      });
...
async function deploy(req, res) {
  ...
  // create deployment package using this code under index.html and zip it
  ...
  // deploy code using netlify
  const netlifyClient = NetlifyAPI(process.env.NETLIFY_ACCESS_TOKEN);

  // deploy the code
  netlifyClient.deploy(site_id, __dirname + "/widget.zip").then(deploy => {
    // update the deployment_url in the widget
    widget.deployment_url = deploy.deploy_url;
